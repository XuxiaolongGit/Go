...
func newRWMockConn(sequence uint8) (*mockConn, *go_sql_driver.mysqlConn) {
	conn := new(mockConn)
	mc := &go_sql_driver.mysqlConn{
		buf:              go_sql_driver.newBuffer(conn),
		cfg:              go_sql_driver.NewConfig(),
		netConn:          conn,
		closech:          make(chan struct{}),
		maxAllowedPacket: go_sql_driver.defaultMaxAllowedPacket,
		sequence:         sequence,
	}
	return conn, mc
}
...
func TestReadPacketWrongSequenceID(t *testing.T) {
	...
	mc.sequence = 1
	_, err := mc.readPacket()
	if err != go_sql_driver.ErrPktSync {
	...
	conn.reads = 0
	mc.sequence = 0
	mc.buf = go_sql_driver.newBuffer(conn)

	// too high sequence id
	conn.data = []byte{0x01, 0x00, 0x00, 0x42, 0xff}
	_, err = mc.readPacket()
	if err != go_sql_driver.ErrPktSyncMul {
...
func TestReadPacketSplit(t *testing.T) {
	...
	data := make([]byte, go_sql_driver.maxPacketSize*2+4*3)
	const pkt2ofs = go_sql_driver.maxPacketSize + 4
	const pkt3ofs = 2 * (go_sql_driver.maxPacketSize + 4)
	...
	data[4] = 0x11
	data[go_sql_driver.maxPacketSize+3] = 0x22
	...
	if len(packet) != go_sql_driver.maxPacketSize {
		t.Fatalf("unexpected packet length: expected %d, got %d", go_sql_driver.maxPacketSize, len(packet))
	}
	if packet[0] != 0x11 {
		t.Fatalf("unexpected payload start: expected %x, got %x", 0x11, packet[0])
	}
	if packet[go_sql_driver.maxPacketSize-1] != 0x22 {
		t.Fatalf("unexpected payload end: expected %x, got %x", 0x22, packet[go_sql_driver.maxPacketSize-1])
	}
	...
	data[pkt2ofs+4] = 0x33
	data[pkt2ofs+go_sql_driver.maxPacketSize+3] = 0x44
	...
	if len(packet) != 2*go_sql_driver.maxPacketSize {
		t.Fatalf("unexpected packet length: expected %d, got %d", 2*go_sql_driver.maxPacketSize, len(packet))
	}
	if packet[0] != 0x11 {
		t.Fatalf("unexpected payload start: expected %x, got %x", 0x11, packet[0])
	}
	if packet[2*go_sql_driver.maxPacketSize-1] != 0x44 {
		t.Fatalf("unexpected payload end: expected %x, got %x", 0x44, packet[2*go_sql_driver.maxPacketSize-1])
	}
	...
	if len(packet) != go_sql_driver.maxPacketSize+42 {
		t.Fatalf("unexpected packet length: expected %d, got %d", go_sql_driver.maxPacketSize+42, len(packet))
	}
	if packet[0] != 0x11 {
		t.Fatalf("unexpected payload start: expected %x, got %x", 0x11, packet[0])
	}
	if packet[go_sql_driver.maxPacketSize+41] != 0x44 {
		t.Fatalf("unexpected payload end: expected %x, got %x", 0x44, packet[go_sql_driver.maxPacketSize+41])
	}
}
