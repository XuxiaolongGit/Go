...
func TestScanNullTime(t *testing.T) {
	var scanTests = []struct {
		...
		{go_sql_driver.tDate, false, true, go_sql_driver.tDate},
		{go_sql_driver.sDate, false, true, go_sql_driver.tDate},
		{[]byte(go_sql_driver.sDate), false, true, go_sql_driver.tDate},
		{go_sql_driver.tDateTime, false, true, go_sql_driver.tDateTime},
		{go_sql_driver.sDateTime, false, true, go_sql_driver.tDateTime},
		{[]byte(go_sql_driver.sDateTime), false, true, go_sql_driver.tDateTime},
		{go_sql_driver.tDate0, false, true, go_sql_driver.tDate0},
		{go_sql_driver.sDate0, false, true, go_sql_driver.tDate0},
		{[]byte(go_sql_driver.sDate0), false, true, go_sql_driver.tDate0},
		{go_sql_driver.sDateTime0, false, true, go_sql_driver.tDate0},
		{[]byte(go_sql_driver.sDateTime0), false, true, go_sql_driver.tDate0},
		{"", true, false, go_sql_driver.tDate0},
		{"1234", true, false, go_sql_driver.tDate0},
		{0, true, false, go_sql_driver.tDate0},
	}

	var nt = go_sql_driver.NullTime{}
	var err error
...
func TestLengthEncodedInteger(t *testing.T) {
	...
	for _, tst := range integerTests {
		num, isNull, numLen := go_sql_driver.readLengthEncodedInteger(tst.encoded)
		if isNull {
		...
		encoded := go_sql_driver.appendLengthEncodedInteger(nil, num)
...
func TestFormatBinaryDateTime(t *testing.T) {
	...
	expect := func(expected string, inlen, outlen uint8) {
		actual, _ := go_sql_driver.formatBinaryDateTime(rawDate[:inlen], outlen)
...
func TestFormatBinaryTime(t *testing.T) {
	expect := func(expected string, src []byte, outlen uint8) {
		actual, _ := go_sql_driver.formatBinaryTime(src, outlen)
		bytes, ok := actual.([]byte)
		if !ok {
			t.Errorf("formatBinaryDateTime must return []byte, was %T", actual)
		}
...
func TestEscapeBackslash(t *testing.T) {
	expect := func(expected, value string) {
		actual := string(go_sql_driver.escapeBytesBackslash([]byte{}, []byte(value)))
		if actual != expected {
			t.Errorf(
				"expected %s, got %s",
				expected, actual,
		...
		actual = string(go_sql_driver.escapeStringBackslash([]byte{}, value))
		if actual != expected {
			t.Errorf(
				"expected %s, got %s",
				expected, actual,
...
func TestEscapeQuotes(t *testing.T) {
	expect := func(expected, value string) {
		actual := string(go_sql_driver.escapeBytesQuotes([]byte{}, []byte(value)))
		if actual != expected {
			t.Errorf(
				"expected %s, got %s",
				expected, actual,
		...
		actual = string(go_sql_driver.escapeStringQuotes([]byte{}, value))
		if actual != expected {
			t.Errorf(
				"expected %s, got %s",
				expected, actual,
...
func TestAtomicBool(t *testing.T) {
	var ab go_sql_driver.atomicBool
	if ab.IsSet() {
...
func TestAtomicError(t *testing.T) {
	var ae go_sql_driver.atomicError
	if ae.Value() != nil {
		t.Fatal("Expected value to be nil")
	}

	ae.Set(go_sql_driver.ErrMalformPkt)
	if v := ae.Value(); v != go_sql_driver.ErrMalformPkt {
		if v == nil {
			t.Fatal("Value is still nil")
		}
		t.Fatal("Error did not match")
	}
	ae.Set(go_sql_driver.ErrPktSync)
	if ae.Value() == go_sql_driver.ErrMalformPkt {
		t.Fatal("Error still matches old error")
	}
	if v := ae.Value(); v != go_sql_driver.ErrPktSync {
		t.Fatal("Error did not match")
	}
}
...
func TestIsolationLevelMapping(t *testing.T) {
	...
	for i, td := range data {
		if actual, err := go_sql_driver.mapIsolationLevel(td.level); actual != td.expected || err != nil {
	...
	actual, err := go_sql_driver.mapIsolationLevel(driver.IsolationLevel(sql.LevelLinearizable))
	if actual != "" || err == nil {
